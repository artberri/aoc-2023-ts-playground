...
const calculateAlternativeHands = (hand: Hand): Hand[] | undefined => {
	const joker = "J";
	const cards = hand.cards.split("");
	const jokerIndex = cards.indexOf(joker);
	const alternativeHands: Hand[] = [];
	if (jokerIndex === -1) {
		return undefined;
	}

	for (let i = 1; i < pokerCardStrengthOrder.length; i++) {
		const alternativeCards = [...cards];
		alternativeCards[jokerIndex] = pokerCardStrengthOrder[i] ?? "J";

		alternativeHands.push({
			...hand,
			cards: alternativeCards.join(""),
		});
	}

	return alternativeHands;
};

const calculateAllAlternativeHands = (hand: Hand): Hand[] => {
	const alternativeHands = calculateAlternativeHands(hand);
	if (!alternativeHands) {
		return [hand];
	}

	const hands: Hand[] = [];
	for (const alternativeHand of alternativeHands) {
		hands.push(...calculateAllAlternativeHands(alternativeHand));
	}

	return hands;
};

const calculateBestHandType = (hand: Hand): HandType => {
	const alternativeHands = calculateAllAlternativeHands(hand);
...
export default (input: string): string => {
	const bid = lines(input)
		...
		.map((hand) => ({
			...hand,
			type: calculateBestHandType(hand),
		}))
		.sort(sortHands)
		.reduce(sumBid, 0);
